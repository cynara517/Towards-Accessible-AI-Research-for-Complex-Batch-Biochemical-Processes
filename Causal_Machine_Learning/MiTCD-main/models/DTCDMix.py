...
from models.CAGKE import CAGKE_1,CAGKE_fix,CAGKE_learnable
...
class CAGKE_U(nn.Module):
    ...
    def __init__(self,length,embed_dim,sigma_min=0.4,sigma_max=4,noise_sigma=0.01,norm_flag=True):
        ...
        self.CAGKE_unit= CAGKE_learnable(in_length=length, embed_dim=embed_dim,sigma_min=sigma_min,
...
class CAGKE_Multi(nn.Module):
    ...
    def __init__(self,length,num_dis,embed_dim,sigma_min=0.4,sigma_max=4,noise_sigma=0.01,norm_flag=True):
        ...
        self.CAGKE_units= nn.ModuleList([CAGKE_learnable(in_length=length, embed_dim=embed_dim,sigma_min=sigma_min,
                                 sigma_max=sigma_max,noise_sigma = noise_sigma) for _ in range(num_dis)])
...
class DREC(nn.Module):
    ...
    def forward(self, X,type_flag):
        ...
        '''
        # X_con=self.CAGKE(X,type_flag)
        X_recons = torch.zeros([X.shape[0],X.shape[1]-self.lag+1,len(type_flag)-np.sum(type_flag),2]).to(X.device)
        ...
        for i in range(len(type_flag)):
            if type_flag[i] == 0:
                ...
                X_recons[:,:,[ind],:] = self.networks[ind](X[:,:,[ind]])
                ind = ind+1
...
def pre_train_model_v2(cagke,cmlp,drec,X,type_flag, lr, max_iter,div_bound=5.0, multi_cagke = False, rec_lambda=0.1, lam=0, lam_ridge=0, penalty='H',sigma_div_weight=0.01,
    ...
    for it in range(max_iter):
        ...
        if multi_cagke:
            cagke_sigma_list.append(cagke.CAGKE_units[0].learnable_sigma.data.detach().cpu().numpy())

            cagke_weight_list.append(cagke.CAGKE_units[0].learnable_weight.data.detach().cpu().numpy())

        else:
            cagke_sigma_list.append(cagke.CAGKE_unit.learnable_sigma.data.detach().cpu().numpy())
        ...
        div_loss_cal = nn.MSELoss()

        if multi_cagke:
            # ensure that sigma > 0
            half = int(cagke.CAGKE_units[0].embed_dim / 2)
            band_div_loss = div_loss_cal(sum([torch.var(torch.abs(unit.learnable_sigma))
                                              for unit in cagke.CAGKE_units])/(len(type_flag)-np.sum(type_flag)),torch.tensor(div_bound).double().to(X.device))

        else:
            # ensure that sigma > 0
            half = int(cagke.CAGKE_unit.embed_dim / 2)
            band_div_loss = div_loss_cal(torch.var(torch.abs(cagke.CAGKE_unit.learnable_sigma)),torch.tensor(div_bound).double().to(X.device))


        loss = forecast_loss + rec_lambda*recon_loss + sigma_div_weight * band_div_loss

        # print(forecast_loss.dtype)
        # print(recon_loss.dtype)
        # print(band_div_loss.dtype)
        # print(loss.dtype)
        ...
        if (it + 1) % check_every == 0:
            ...
            recon_acc_list.append(dis_recon_pre.clone().eq(drec(X_con,type_flag).clone().argmax(dim=-1)).float().mean().detach())
            ...
            if verbose > 0:
                ...
                print('reconstruction_acc = %f' % dis_recon_pre.clone().eq(drec(X_con,type_flag).clone().argmax(dim=-1)).float().mean().detach().cpu().numpy())
    ...
    return train_loss_list,forecast_loss_list,recons_loss_list,div_loss_list,recon_acc_list,cagke_sigma_list,cagke_weight_list
